...
const Management: React.FC<ManagementProps> = ({ 
  ...
  const [viewingItem, setViewingItem] = useState<{ type: 'category' | 'merchant' | 'payment', data: any } | null>(null);
  ...
  // Handle auto-viewing merchant detail if requested from outside via targetMerchantName prop
  useEffect(() => {
    if (mode === 'merchants' && targetMerchantName) {
      const m = merchants.find(merch => merch.name.toLowerCase() === targetMerchantName.toLowerCase());
      if (m) {
        setViewingItem({ type: 'merchant', data: m });
        onClearTargetMerchant?.();
      }
    }
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      ...
      if (sortKey === 'name') {
        ...
        valA = merchantStatsMap[nameA]?.lastDate || '';
        valB = merchantStatsMap[nameB]?.lastDate || '';
      }
  ...
  const handleAddSub = (cat: Category) => {
    ...
    if (!trimmed || !cat.id) return;
    const subs = cat.subCategories || [];
    if (subs.includes(trimmed)) { setIsAddingSub(false); setNewSubName(''); return; }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-gray-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className={`p-6 flex items-center justify-between border-b border-gray-100 ${
              ...
              <div className="flex items-center gap-2">
                <button 
                  onClick={() => { openEdit(viewingItem.type, viewingItem.data); setViewingItem(null); }}
            ...
            <div className="p-8 space-y-8">
              <div className="flex flex-col items-center text-center pb-6 border-b border-gray-100">
                ...
                <h2 className="text-4xl font-black text-gray-900 uppercase tracking-tight">{viewingItem.data.name}</h2>
              </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                      ...
                      <p className="text-sm font-black text-blue-600 uppercase tracking-tight">{viewingItem.data.type || 'BOTH'}</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                      <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Total Volume</p>
                      <p className="text-sm font-black text-gray-900">${(categoryStatsMap[viewingItem.data.name.toLowerCase()] || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                  </div>
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {!isAddingSub && (
                        <button onClick={() => setIsAddingSub(true)} className="p-1 text-blue-600 hover:bg-blue-50 rounded"><Plus size={14} /></button>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {viewingItem.data.subCategories?.map((s: string) => (
                        <span key={s} className="px-3 py-1 bg-white border border-gray-100 rounded-lg text-[11px] font-bold text-gray-600 flex items-center gap-2">
                          {s}
                          <button onClick={() => onDeleteSubCategory(viewingItem.data.id, s)} className="text-gray-300 hover:text-rose-500"><X size={10} /></button>
                        </span>
                      ))}
                      {isAddingSub && (
                        <div className="flex items-center gap-1 bg-white border border-blue-200 rounded-lg px-2 py-1 shadow-sm">
                          <input autoFocus value={newSubName} onChange={(e) => setNewSubName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddSub(viewingItem.data)} className="text-[11px] font-bold outline-none w-24" placeholder="Name..." />
                          <button onClick={() => handleAddSub(viewingItem.data)} className="p-0.5 text-blue-600"><Check size={12} /></button>
                          <button onClick={() => setIsAddingSub(false)} className="p-0.5 text-gray-400"><X size={12} /></button>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                      ...
                      <p className="text-sm font-black text-gray-900">{merchantStatsMap[viewingItem.data.name.toLowerCase()]?.count || 0}</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                      <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Last Seen</p>
                      <p className="text-sm font-black text-gray-900">{merchantStatsMap[viewingItem.data.name.toLowerCase()]?.lastDate || 'Never'}</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 text-sm font-bold text-gray-600">
                      <MapPin size={16} className="text-rose-500" /> {viewingItem.data.location || 'No location set'}
                    </div>
                    <div className="flex items-center gap-3 text-sm font-bold text-gray-600">
                      <Phone size={16} className="text-emerald-500" /> {viewingItem.data.phone || 'No phone set'}
                    </div>
                    {viewingItem.data.website && (
                      <a href={viewingItem.data.website} target="_blank" rel="noreferrer" className="flex items-center gap-3 text-sm font-black text-blue-600 hover:underline">
                        <Globe size={16} /> Website <ExternalLink size={12} />
                      </a>
                    )}
                  </div>
                  <button 
                    onClick={() => { onViewMerchantTransactions?.(viewingItem.data.name); setViewingItem(null); }}
                    className="w-full py-3 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-indigo-100"
                  >
              ...
              {viewingItem.type === 'payment' && (
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 text-center">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Total Volume</p>
                    <h5 className="text-xl font-black text-emerald-600">${(paymentStatsMap[viewingItem.data.name.toLowerCase()]?.total || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h5>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 text-center">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Frequency</p>
                    <h5 className="text-xl font-black text-blue-600">{paymentStatsMap[viewingItem.data.name.toLowerCase()]?.count || 0} Transactions</h5>
                  </div>
                </div>
              )}
              ...
              <div className="flex flex-col gap-3 pt-4">
                <button 
                  onClick={() => { setDeletingItem({ type: viewingItem.type, data: viewingItem.data }); setViewingItem(null); }}
                  className="w-full py-4 bg-rose-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 hover:bg-rose-700 transition-all"
                >
                  <Trash2 size={14} /> Delete {viewingItem.type}
                </button>
                <button 
                  onClick={() => setViewingItem(null)}
                  className="w-full py-4 bg-gray-100 text-gray-500 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-gray-200 transition-all"
                >
                  Dismiss
                </button>
              </div>
      ...
      {/* Universal Edit Modal - Resized to max-w-2xl */}
      {editingItem && editingItem.type !== 'subCategory' && (
        <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4" onClick={() => setEditingItem(null)}>
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-gray-100 overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <input 
                  ...
                  className={`w-full px-4 py-3 bg-gray-50 border ${!editName.trim() ? 'border-rose-200' : 'border-gray-200'} rounded-xl font-bold focus:ring-2 focus:ring-blue-100 outline-none transition-all`} 
                />
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredAndSortedItems.map(cat => {
                    ...
                    return (
                      <tr key={cat.id} className="hover:bg-blue-50/20 group transition-colors cursor-pointer" onClick={() => setViewingItem({ type: 'category', data: cat })}>
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredAndSortedItems.map(m => {
                    const stats = merchantStatsMap[m.name.toLowerCase()] || { total: 0, count: 0, lastDate: null };
                    const perc = totalMerchantsSum > 0 ? (stats.total / totalMerchantsSum) * 100 : 0;
                    return (
                      <tr key={m.id} className="hover:bg-blue-50/20 group transition-colors cursor-pointer" onClick={() => setViewingItem({ type: 'merchant', data: m })}>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredAndSortedItems.map(p => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-blue-50/20 group transition-colors cursor-pointer" onClick={() => setViewingItem({ type: 'payment', data: p })}>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
